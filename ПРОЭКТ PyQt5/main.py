...
class Setting(QWidget):
    ...
    def init_ui(self):
        ...
        cur = self.con.cursor()
        col = cur.execute("SELECT val FROM settings WHERE name='bg color main window'").fetchall()
    ...
    def save_settings(self):
        cur = self.con.cursor()
        sql_color = "UPDATE settings SET val=? WHERE name='bg color main window'", (self.color, )
...
class Base(QDialog):
    ...
    def init(self):
        ...
        self.add_btn.setText(self.language_words[50])
        self.del_btn.setText(self.language_words[51])
        self.take_btn.setText(self.language_words[52])

        self.setStyleSheet("#Form{background-color:Black}")
        self.move(500, 200)
...
class Main(QMainWindow):
    ...
    def mousePressEvent(self, event):
        if self.tool in ['marker', 'stylus', 'brush'] or self.tool in ['маркер', 'стилус', 'кисть']:
        ...
        elif self.tool == 'line' or self.tool == 'линия':
            ...
            if event.x() in [i for i in range(self.x, self.x + self.width)] and \
                ...
                y = u"Y\u2081"
                self.x_y_label.setText(f'{x}: {event.x() - self.x}, {y}: {event.y() - self.y}.')
        elif self.tool in ['rectangle', 'circle'] or self.tool in ['прямоугольник', 'круг']:
    ...
    def mouseMoveEvent(self, event):
        if self.mouse_pressed:
            if self.tool in ['marker', 'stylus', 'brush', 'rubber'] or self.tool in ['маркер', 'стилус',
                                                                                     'кисть', 'ластик']:
                self.move_point(event)
                if event.x() in [i for i in range(self.x, self.x + self.width)] and \
                        event.y() in [i for i in range(self.y, self.y + self.height)]:
                    self.x_y_label.setText(f'X: {event.x() - self.x}, Y: {event.y() - self.y}.')
            elif self.tool in ['rectangle', 'circle', 'line'] or self.tool in ['прямоугольник', 'круг', 'линия']:
                if event.x() in [i for i in range(self.x, self.x + self.width)] and \
                        event.y() in [i for i in range(self.y, self.y + self.height)]:
                    x = u"X\u2081"
                    y = u"Y\u2081"
            ...
            try:
                if self.events[i][0] == 'Point':
                    n = QColor(self.events[i][1][0], self.events[i][1][1],
                               self.events[i][1][2], self.events[i][2])                   # 255
                    cords = [self.events[i][4]]
    ...
    def line_style(self):
        stl = None
        if self.style == 'SolidLine' or self.style == 'Сплошная линия':
            stl = Qt.SolidLine
        elif self.style == 'DashLine'or self.style == 'Тире линия':
            stl = Qt.DashLine
        elif self.style == 'DotLine'or self.style == 'Точечная линия':
            stl = Qt.DotLine
        elif self.style == 'DashDotLine'or self.style == 'Пунктирная линия':
            stl = Qt.DashDotLine
        return stl
    ...
    def init_ui(self):
        ...
        menu = menubar.addMenu(f'&{self.language_words[15]}')
        file = menubar.addMenu(f'&{self.language_words[16]}')
        settings = menubar.addMenu(f'&{self.language_words[14]}')
        ...
        self.git.setText(f'<a href="https://github.com/VladimirBadzgaradze/PyQt5-project">{self.language_words[19]}</a>')
